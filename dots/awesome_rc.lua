...
local memP = lain.widget.mem {
	settings = function()
		widget:set_markup(mem_now.perc .. "%")
	end
}
local memMB = lain.widget.mem {
	settings = function()
...
local battery = lain.widget.bat {
    settings = function()
        local acstatus = bat_now.ac_status
        local ac
        if(acstatus == 1) then
            ac = "C"
        else
        ...
        widget:set_markup(bat_now.perc .. "% ".. bat_now.time .." (" .. ac ..")")
    end
}
--
...
clientkeys = gears.table.join(
    ...
    awful.key({modkey, "Shift" }, "Up", function (c) c:relative_move(0, -10, 0, 0) end,
              {description = "Move client up", group = "client"}),
    awful.key({modkey, "Shift" }, "Down", function (c) c:relative_move(0,10,0,0) end,
              {description = "Move client down", group = "client"}),
    awful.key({modkey, "Shift" }, "Left", function (c) c:relative_move(-10,0,0,0) end,
              {description = "Move client left", group = "client"}),
    awful.key({modkey, "Shift" }, "Right", function (c) c:relative_move(10,0,0,0) end,
